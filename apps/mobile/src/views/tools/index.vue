...
<template>
  <div class="tools-content pt-[20px] px-[12px]">
    ...
    <div class="pl-[12px] border-l-[3px] border-[color:#41b883] mb-[12px]">
      <h3 class="font-bold text-[18px] my-[4px]">HTTP 请求示例</h3>
    </div>

    <!-- GET -->
    <p class="text-[13px] text-[var(--van-text-color-2)] mb-[6px]">
      <van-tag type="success" class="mr-[4px]" plain>GET</van-tag>
      获取用户列表
    </p>
    <van-button
      type="success"
    ...
    >
      <div v-for="user in userList" :key="user.id" class="flex items-center py-[6px]">
        <van-image :src="user.avatar" round width="32" height="32" class="mr-[10px] shrink-0" />
    ...
    <p class="text-[13px] text-[var(--van-text-color-2)] mt-[16px] mb-[6px]">
      <van-tag type="primary" class="mr-[4px]" plain>POST</van-tag>
      提交表单数据
    </p>
    <van-button
      type="primary"
    ...
    <!-- 错误请求 -->
    <p class="text-[13px] text-[var(--van-text-color-2)] mt-[16px] mb-[6px]">
      <van-tag type="danger" class="mr-[4px]" plain>ERROR</van-tag>
      模拟请求失败（拦截器自动处理）
    </p>
    <van-button type="danger" size="small" @click="handleErrorRequest">发起失败请求</van-button>
    <!-- Icon -->
