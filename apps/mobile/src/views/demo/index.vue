...
<template>
  ...
  <div class="demo-content px-[12px]">
    ...
    <div class="mt-[16px] pb-[24px] grid grid-cols-2 gap-[12px] text-[14px] text-center">
      <div
        v-for="item in contentList"
        :key="item.text"
        class="truncate p-[12px] rounded-[12px] border border-[var(--color-border)]"
        :class="[item.fullWidth ? 'col-span-2' : '']"
      >
        {{ item.text }}
      </div>
    </div>
